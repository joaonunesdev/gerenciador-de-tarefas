...
const auth = async (req, res, next) => {
    try {
        ...
        const usuario = await Usuario.findOne({
            _id: decoded._id,
            'tokens.token': token
        }); // Busca por um usuário que possua a _id informada e que ainda possua o token na array de tokens 
        ...
        req.usuario = usuario; // Adiciona o usuário na requisição
        next(); // A requisição prossegue 
    } catch (e) {
        res.status(401).send({
            error: 'Usuário precisa estar autenticado no sistema.'
        }); // Captura o erro
    }
}
